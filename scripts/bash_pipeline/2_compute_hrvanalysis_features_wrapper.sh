...
if [[ $InputDest ]] || [[ $TargetDest ]]; then
  echo "Start Executing script"
    ...
    echo "No Input res directory: $InputDest or Target directory: $TargetDest." >&2
...
for rr_intervals_file in $(find $InputDest/* -type f -name "*.csv" ); do
    ...
    python3 $ECG_PATH/src/usecase/compute_hrvanalysis_features.py \
	    --rr-intervals-file-path $rr_intervals_file \
