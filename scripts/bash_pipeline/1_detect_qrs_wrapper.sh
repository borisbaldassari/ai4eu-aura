...
# Option - select input directory to be copied from and output directory to
# copy into
...
if [[ $InputDest ]] || [[ $TargetDest ]]; then
  echo "Start Executing script"
    ...
    echo "No Input directory: $InputDest or Target directory: $TargetDest." >&2
    echo "Use -i,-o options" >&2
  exit 1
fi
...
for edf_file in $(find $InputDest/* -type f -name "*.edf" ); do
    ...
    python3 $ECG_PATH/src/usecase/detect_qrs.py \
	    --qrs-file-path $edf_file --method hamilton \
	    --output-folder $TargetDest/$relative_path
