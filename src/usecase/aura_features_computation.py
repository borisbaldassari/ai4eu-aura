...
def get_clean_intervals(rrs):
    ...
    rr_intervals_without_outliers = remove_outliers(
        rr_intervals=rrs, low_rri=300, high_rri=1800
    )
...
def compute_short_term_features_on_interval(features, i, rr_timestamps, rrs):
    ...
    rrs_on_interval = get_rr_intervals_on_window(
        rr_timestamps, rrs, i * SHORT_WINDOW, SHORT_WINDOW
    )
...
def compute_medium_term_features_on_interval(
    features, i, rr_timestamps, rrs, medium_window_offset
):

    if (i * SHORT_WINDOW) > MEDIUM_WINDOW:
        rr_on_medium_intervals = get_rr_intervals_on_window(
            rr_timestamps, rrs, (i - medium_window_offset) * SHORT_WINDOW, MEDIUM_WINDOW
        )
...
def compute_long_term_features_on_interval(
    features, i, rr_timestamps, rrs, large_window_offset
):

    if (i * SHORT_WINDOW) > LARGE_WINDOW:
        rr_on_large_intervals = get_rr_intervals_on_window(
            rr_timestamps, rrs, (i - large_window_offset) * SHORT_WINDOW, LARGE_WINDOW
        )
...
def compute_features(
    ...
    try:
        ...
        for i in range(0, n_short_intervals):
            try:
                compute_labels_on_interval(
                    features, i, background_intervals, seizure_intervals
                )
            ...
            try:
                compute_medium_term_features_on_interval(
                    features, i, rr_timestamps, rrs, medium_window_offset
                )
            ...
            try:
                compute_long_term_features_on_interval(
                    features, i, rr_timestamps, rrs, medium_window_offset
                )
        ...
        data = {"keys": keys, "features": features.tolist()}
...
if __name__ == "__main__":
    ...
    if args.qrs_detector_used.lower() not in [
        "gqrs",
        "xqrs",
        "hamilton",
        "engelsee",
